...
def execute():
	matching(translate(file2array(filename))) #Finds meaning of braille2
	...
	for word in z:
		for m in re.finditer("[a-zA-Z\s]*?\d\w?", word):
			if re.search('[a-z]*', m.group()) != None:
				tag = (re.search('[a-z]*', m.group())).group()
				for n in re.finditer('\d{1}', m.group()): #checks if unknown has already been found
					duplicates1.append(n.group()) #array of identified unknowns
					if duplicates1.count(n.group()) != 1: break #if in array, break
					else: 
					
						dict[dictU[n.group()]] = e[index_of_word].replace(tag, '') #if not in array,wait
